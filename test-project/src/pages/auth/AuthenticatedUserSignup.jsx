...
function AuthenticatedUserSignup() {
  const { isInitialized } = useAuth()
  const { user } = useSelector(state => state.user)
  const navigate = useNavigate()
  
  // Hardcode profile slug from profiles.json - Case sensitive
  const profileSlug = "|khEVo8m62LxaEEGDfyiLfQ=="
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      if(!user) {
        ...
        ApperUI.showSignup("#authentication", { profileSlug })
      } else {
      ...
      <div className="w-full max-w-md space-y-8 p-8 bg-white dark:bg-surface-800 rounded-lg shadow-md">
        ...
        <div className="text-center mt-4">
          <p className="text-sm text-surface-600 dark:text-surface-400">
            ...
            <Link to="/authenticateduser/login" className="font-medium text-primary hover:text-primary-dark">
              Sign in
            </Link>
          </p>
        </div>
...
export default AuthenticatedUserSignup